...
def check_for_captcha(page: Page):
    if page.locator("id=challenge-form").is_visible():
        print("Captcha found!")
        print("Waiting for captcha to be solved...")
        page.wait_for_selector("id=challenge-form", state="hidden")
...
# * GENERATE PIN
pin = FAKE.random_int(min=1000)

# * GENERATE DATE
today = date.today()
birthdate = FAKE.date(
    pattern="%d-%m-%Y", end_datetime=date(today.year - 18, today.month, today.day)
)
...
with sync_playwright() as playwright:
    ...
    email_page.goto(TEMP_EMAIL_URL, wait_until="load")
    ...
    register_page.goto(OSRS_REGISTER_URL, wait_until="load")

    # * CHECK IF REACHABLE
    if (
        register_page.locator('text="The web server is not reachable"').is_visible()
        or register_page.locator(
            'text="An error has occurred and it has not been possible to create your account."'
        ).is_visible()
    ):
    ...
    terms_button.check()
    submit_button.click()
    register_page.wait_for_load_state("load")

    # * CAPTCHA
    check_for_captcha(register_page)

    # * CHECKING FOR SUCCESS
    if register_page.locator(
        'text="You can now begin your adventure with your new account."'
    ).is_visible():
        account_made = True
...
display_bot_info(username, password, email, birthdate, pin)

if account_made:
    write_bot_data(username, password, email, pin)
